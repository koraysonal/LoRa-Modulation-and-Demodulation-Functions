...
lengthfirtssignal=length(signal)
if nargin == 6
    SNR                 = Inf ;
    n_preamble          = 8 ;
elseif nargin == 7
...
if Fs == Bandwidth
    ...
    signal_freq_demod   = signal.*exp(+1i.*2.*pi.*df./Fs.*(0:length(signal)-1))' ;
    length_freq_demodsignal=length(signal_freq_demod )
    figure(4)
    freqqq=abs(fft(signal_freq_demod)).^2;
    f = linspace(0,Fs,length(freqqq)); % Frekans vektörü
    n = floor(length(f)/2); % Vektörün uzunluğunun yarısı (buçuklu çıkma
                            % ihtimaline karşı en küçük tamsayıyı aldık.
                            % Dönüşümün büyüklüğünü çizdir

    plot(f(1:n),(freqqq(1:n)),'LineWidth',2);
    xlabel('Frekans (Hz)');
    grid;
    title('after multiply exp');
    
    signal_filter       = lowpass(signal_freq_demod,Bandwidth,Fs) ;
    length_signalafterfilter=length(signal_filter )
    figure(5)
    freqqq=abs(fft(signal_filter  )).^2;
    f = linspace(0,Fs,length(freqqq)); % Frekans vektörü
    n = floor(length(f)/2); % Vektörün uzunluğunun yarısı (buçuklu çıkma
                            % ihtimaline karşı en küçük tamsayıyı aldık.
                            % Dönüşümün büyüklüğünü çizdir
    ...
    signal_demod        = awgn(resample(signal_filter,Bandwidth,Fs),SNR,'measured') ; %% BW  yeni örnekleme frekansı bunla sinyali çarpıp datamın boyutnu düşünrdüm .ns düştü
    length_signaldemodend=length(signal_demod )
    figure(6)
    freqqq=abs(fft(signal_demod )).^2;
    f = linspace(0,Fs,length(freqqq)); % Frekans vektörü
    n = floor(length(f)/2); % Vektörün uzunluğunun yarısı (buçuklu çıkma
    % ihtimaline karşı en küçük tamsayıyı aldık.
    % Dönüşümün büyüklüğünü çizdir
...
%% Demodualte and Extract Preamble
    ...
    Nsymbols        = floor(length(signal)/M)  %% toplam sembol sayısı (chirp) Örneklemeyi değiştirdiğimiz için M e bölündü

    UChirpsDemod    = loramod(zeros(1,Nsymbols),SF,Bandwidth,Bandwidth);  % time =Ts oldu sembol miktarı kadar chirp oluştu
    figure(7)
    spectrogram(UChirpsDemod,500,0,500,Bandwidth,'yaxis','centered')

    legnthuchirp=length(UChirpsDemod)
    figure(8)
    freqqq=abs(fft(UChirpsDemod )).^2;
    f = linspace(0,Bandwidth,length(freqqq)); % Frekans vektörü
    n = floor(length(f)/2); % Vektörün uzunluğunun yarısı (buçuklu çıkma
                               % ihtimaline karşı en küçük tamsayıyı aldık.
                            % Dönüşümün büyüklüğünü çizdir
    ...
    xlabel('Frekans (Hz)');
    grid;
    title('Uchirp fourier');

    SniffSignal     = signal(1:length(UChirpsDemod)).*UChirpsDemod ; % gelen sinyal ile upchirp carpıldı
    figure(9)
    spectrogram(SniffSignal,500,0,500,Bandwidth,'yaxis','centered')
    length_sniffsignal=length(SniffSignal)
    figure(10)
    freqqq=abs(fft(SniffSignal )).^2;
    f = linspace(0,Bandwidth,length(freqqq)); % Frekans vektörü
    n = floor(length(f)/2); % Vektörün uzunluğunun yarısı (buçuklu çıkma
                            % ihtimaline karşı en küçük tamsayıyı aldık.
                            % Dönüşümün büyüklüğünü çizdir
    ...
    xlabel('Frekans (Hz)');
    grid;
    title('sniffsignal fourier');
if Coherece == 2 %% non-cohorent için preamble hesaplama
    
    fftSync     = fft(reshape(SniffSignal,M,length(SniffSignal)/M)) ;% M=2^SF (toplam oluşturabilicek sembol sayısı) örneğin 1024 35 oluştu
    [~,SyncInd] = sort(max(fftSync)) ;% bu 35 tane sutundan max olanların indisleri alındı
    sync        = sort(SyncInd(end-1:end)) ;
    sync        = sync(end) + 1 ;
    NPreamb     = sync - 5 ;
else%% cohorent için preamble hesaplama
    NPreamb     = n_preamble ;
end
dChirpsDemod    = loramod(zeros(1,NPreamb),SF,Bandwidth,Bandwidth,-1) ;%% preamble miktarı kadar downchirp oluştu
figure(11)
freqqq=abs(fft(dChirpsDemod  )).^2;
f = linspace(0,Bandwidth,length(freqqq)); % Frekans vektörü
n = floor(length(f)/2); % Vektörün uzunluğunun yarısı (buçuklu çıkma
% ihtimaline karşı en küçük tamsayıyı aldık.
% Dönüşümün büyüklüğünü çizdir
...
xlabel('Frekans (Hz)');
grid;
title('Dchirp Spectrum');

pream_signal    = signal(1:length(dChirpsDemod)).*dChirpsDemod ;%% gelen sinyalim ile oluşturdugum dowchirp i çarpıldı böylece preamble kısmı demodüalyon oldu
figure(12)
freqqq=abs(fft(pream_signal  )).^2;
f = linspace(0,Bandwidth,length(freqqq)); % Frekans vektörü
n = floor(length(f)/2); % Vektörün uzunluğunun yarısı (buçuklu çıkma
% ihtimaline karşı en küçük tamsayıyı aldık.
% Dönüşümün büyüklüğünü çizdir
...
figure(13)
freqqq=abs(fft(symbols_pream  )).^2;
f = linspace(0,Bandwidth,length(freqqq)); % Frekans vektörü
n = floor(length(f)/2); % Vektörün uzunluğunun yarısı (buçuklu çıkma
% ihtimaline karşı en küçük tamsayıyı aldık.
% Dönüşümün büyüklüğünü çizdir
...
xlabel('Frekans (Hz)');
grid;
title('after fsk detection');

symbol_offset   = mode(symbols_pream) + 1 
%% Demodulate Message
MessageStartInd = (NPreamb + 4.25)*M  %Sinyaldeki Nhp nin başlangıç indexi (Gelen sinyalin ilk kısmı preamble 4.25 de senkronizasyon için var toplam NPreamb + 4.25 byte eder)Burada bir sembolumde(bir byte gibi düşün) 2^SF kadar örnek var(Başta örneklemeyi BW yaptık)(NPreamb + 4.25 byte)*M kadar byte sonra benim mesajım elde edilir headerla başlayan
Nmessage        = floor(length(signal)/M - MessageStartInd/M) %% Nhp (Kaç tane mesajım oldugunu buldum(bilgi taşıyan preamble ve sycn yok) yani sembolüm)(length(signal)=sinyalim kaç byte,M e bölüp kaç mesajım olduğu bulunur)
MessageEndInd   = Nmessage.*M + MessageStartInd %Sinyaldeki Nhp nin bitis indexi
MessageSignal   = signal(MessageStartInd+1:MessageEndInd).*loramod(zeros(1,Nmessage),SF,Bandwidth,Bandwidth,-1) ; %%Gelen sinyalin bilgi yaşıtan kısmı(Nhp) down chirp ile çarpıldı.(sinyalimi bw ile yeniden örnekledim yani fs =bw olduğu için loramodda fs bw oldu)
...
% Symbol Constants
Ts      = 2^SF/BW ;
Ns      = fs.*M/BW ; % bir sembol burada M kadar
gamma   = x/Ts ;
beta    = BW/Ts ;
time    = (0:Ns-1)'.*1/fs ;
