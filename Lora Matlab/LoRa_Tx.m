...
figure(1)
plot(f(1:n),abs(freqqq(1:n)),'LineWidth',2);
...
imp = 0 ;%%H imlicit header flag
opt = 0 ;%%OP optimization mode flag
%% String to Decimal
...
pld_dbl = [255 255 0 0 message_dbl 0 CRC_dbl pad_dbl] ; % LoRa payload  %% CRC dbl 2 byte 

%lengthpacket_payload=length(pld_dbl)

%% Swap Nibbles
pld_swp = LoRa_encode_swap(pld_dbl) ;

%lengthswap_payload=length(pld_swp)

%% Payload Encoding
pld_enc = LoRa_encode_hamming(pld_swp,CR); 
pld_enc = pld_enc(1 : n_wht) ;

%lengthpacket_payload_encoding=length(pld_enc)

%% Payload Whiten
pld_wht = LoRa_encode_white(pld_enc,CR,0) ;

%lengthwhitening=length(pld_wht)

%% Header Encoding
packet_hdr = [(length(message_dbl)+5) CRC_pld*16+(CR==1)*32+(CR==2).*64+(CR==3).*96+(CR==4)*128 224] ;
packet_hdr_enc_tmp = LoRa_encode_hamming(packet_hdr,4); %% Header encode always 4,8
packet_hdr_enc = [packet_hdr_enc_tmp(1:5) pld_wht(1:N_pld-1)] ;

%lengthpacket_header_encoding=length(packet_hdr_enc)
...
%lengthpacket_hdr_shf=length(packet_hdr_shf)
%lengthpacket_pld_shf=length(packet_pld_shf)

% Interleaving
packet_pld_int = LoRa_encode_interleave(packet_pld_shf,SF,CR) ;
packet_hdr_int = LoRa_encode_interleave(packet_hdr_shf,SF-2,4); 

%lengthpacket_hdr_int=length(packet_hdr_int)
%lengthpacket_pld_int=length(packet_pld_int)

% Graying
packet_pld_gray = LoRa_encode_gray(packet_pld_int) ;
packet_hdr_gray = LoRa_encode_gray(packet_hdr_int) ;

%lengthpacket_hdr_gray =length(packet_hdr_gray )
%lengthpacket_pld_gray =length(packet_pld_gray )

% Packet final
packet = [4*packet_hdr_gray packet_pld_gray] ;

lengthpacket=length(packet)
end
...
signal_prmb = loramod((SyncKey - 1).*ones(1,n_preamble),SF,Bandwidth,Fs,1) ; % preamble upchirps      8 tane oluştu upchirp preamble
signal_sync_u = loramod([0 0 ],SF,Bandwidth,Fs,1) ; % sync upchirp                                    % yukarıdaki ile birlikte npr+2 upcihrp 2 burdan Npr yukarısı
signal_sync_d1 = loramod(0,SF,Bandwidth,Fs,-1) ; % header downchirp
signal_sync_d = [signal_sync_d1; signal_sync_d1; signal_sync_d1(1:length(signal_sync_d1)/4)] ; % concatenate header      2.25 down chirp
...
% Symbol Constants
Ts      = 2^SF/BW ;
Ns      = fs.*M/BW; %% bir sembol kaç sampledan oluşuyor
gamma   = x/Ts ;
beta    = BW/Ts ;
time    = (0:Ns-1)'.*1/fs ; 
